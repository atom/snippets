...
describe('Snippet Loading', () => {
  ...
  beforeEach(() => {
    ...
    spyOn(atom.packages, 'getLoadedPackages').andReturn([
      atom.packages.loadPackage(
        path.join(__dirname, 'fixtures', 'package-with-snippets')
      ),
      atom.packages.loadPackage(
        path.join(__dirname, 'fixtures', 'package-with-broken-snippets')
      )
    ])
  })
  ...
  it('loads the bundled snippet template snippets', () => {
    ...
    runs(() => {
      const jsonSnippet = snippetsService.snippetsForScopes(['.source.json'])
        .snip
      ...
      const csonSnippet = snippetsService.snippetsForScopes(['.source.coffee'])
        .snip
  ...
  it('loads non-hidden snippet files from atom packages with snippets directories', () => {
    ...
    runs(() => {
      ...
      expect(snippet.rightLabelHTML).toBe(
  ...
  describe('::onDidLoadSnippets(callback)', () => {
    it('invokes listeners when all snippets are loaded', () => {
      ...
      waitsFor(
        'onDidLoad callback to be called',
        () => loadedCallback.callCount > 0
      )
    })
  })
...
`
        ...
        waitsFor(
          ...
          () => !snippetsService.snippetsForScopes(['.foo']).foo
        )
      })
    })
  })
...
`
  ...
  describe('packages-with-snippets-disabled feature', () => {
    it('disables no snippets if the config option is empty', () => {
      const originalConfig = atom.config.get(
      ...
      activateSnippetsPackage()
      runs(() => {
        const snippets = snippetsService.snippetsForScopes([
    ...
    it('unloads and/or reloads snippets from a package if the config option is changed after activation', () => {
      ...
      activateSnippetsPackage()
      runs(() => {
        let snippets = snippetsService.snippetsForScopes([
          '.package-with-snippets-unique-scope'
        ])
        expect(Object.keys(snippets).length).toBe(1)
        ...
        snippets = snippetsService.snippetsForScopes([
          '.package-with-snippets-unique-scope'
        ])
        expect(Object.keys(snippets).length).toBe(0)
