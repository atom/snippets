...
module.exports = class SnippetExpansion {
  constructor (snippet, editor, cursor, snippets) {
    ...
    const indent = this.editor
      .lineTextForBufferRow(startPosition.row)
      .match(/^\s*/)[0]
  ...
  cursorMoved ({ oldBufferPosition, newBufferPosition, textChanged }) {
    ...
    const itemWithCursor = this.tabStopMarkers[this.tabStopIndex].find(item =>
    ...
    if (ranges.length > 0) {
      ...
      for (let i = 0; i < ranges.length; i++) {
        ...
        if (this.selections[i]) {
          ...
          this.subscriptions.add(
            newSelection.cursor.onDidChangePosition(event =>
              this.cursorMoved(event)
            )
          )
          this.subscriptions.add(
            newSelection.cursor.onDidDestroy(() => this.cursorDestroyed())
          )
