...
module.exports = class SnippetExpansion {
  ...
  cursorMoved ({oldBufferPosition, newBufferPosition, textChanged}) {
    ...
    if (itemWithCursor && itemWithCursor.insertion.choices.length > 0) {
      setTimeout(() => {
  ...
  setTabStopIndex (tabStopIndex) {
    ...
    for (const item of items) {
      ...
      if (insertion.isChoices()) {
        choices = insertion.choices
      }
    ...
    if (choices.length > 0) {
      console.log("Activating choices")
      this.snippets.snippetChoiceProvider.activate(choices)
    } else {
      this.snippets.snippetChoiceProvider.deactivate()
    }
