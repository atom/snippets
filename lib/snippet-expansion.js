...
module.exports = class SnippetExpansion {
  ...
  goToNextTabStop () {
    ...
    if (nextIndex < this.tabStopMarkers.length - this.tabStopList.hasEndStop) { // this cracks me up for some reason :) it's so wrong, but it works 'cause true == 1
      if (this.setTabStopIndex(nextIndex)) {
        return true
      } else {
        return this.goToNextTabStop() // recursively try the next one
      }
    } else {
