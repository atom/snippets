...
module.exports = class Snippet {
  ...
  toString (params = {startPosition: {row: 0, column: 0}, indent: ''}) {
    ...
    const index = addTabstopsForUnknownVariables(acc.unknownVariables, acc.tabStopList)

    if (!acc.tabStopList.hasEndStop && atom.config.get("snippets.implicitEndTabstop")) {
      const endRange = new Range([acc.row, acc.column], [acc.row, acc.column])
      acc.tabStopList.findOrCreate({index, snippet: this}).addInsertion({range: endRange})
    }
