...
module.exports = {
  ...
  textChanged (editor, event) {
    ...
    this.ignoringTextChangesForEditor(editor, () =>
      editor.transact(editor.undoGroupingInterval || 300, () => // HACK: relies on private editor property, and on one that will likely change in future (to non time based undo)
        activeExpansions.map(expansion => expansion.textChanged(event)))
    )

    // NOTE: Making the checkpoint appears to have contributed to the transformation undo issues
  ...
  consumeVariableResolver (resolver) {
