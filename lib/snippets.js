...
module.exports = {
  ...
  goToNextTabStop (editor) {
    ...
    for (const expansion of this.getExpansions(editor)) {
      ...
      const {succeeded, end} = expansion.goToNextTabStop()
      if (isEnd) { isEnd = end }
    ...
    console.log('end:', isEnd, 'succ:', nextTabStopVisited)
  ...
  goToPreviousTabStop (editor) {
    ...
    for (const expansion of this.getExpansions(editor)) {
      if (!expansion) { continue; }
      const {succeeded, end} = expansion.goToPreviousTabStop(!previousTabStopVisited)
      // if (isEnd) { isEnd = end }
