...
module.exports = {
  ...
  get bodyParser () {
    ...
    const grammar = fs.readFileSync(require.resolve('./snippet-body-parser.pegjs'), 'utf8')
  ...
  add (filePath, snippetsBySelector, isDisabled = false) {
    for (const selector in snippetsBySelector) {
      ...
      for (const identifier in snippetsByName) {
        const attributes = snippetsByName[identifier]
  ...
  getParsedSnippet ({ id, body, bodyTree, ...attributes }) {
    if (!this.parsedSnippetsById.has(id)) {
      this.parsedSnippetsById.set(id, new Snippet({
        ...(bodyTree || this.bodyParser.parse(body)),
        ...attributes
      }))
    }
    return this.parsedSnippetsById.get(id)
  },
  ...
  insert (snippet, editor, cursor) {
    ...
    if (typeof snippet === 'string') {
      snippet = new Snippet({ ...this.bodyParser.parse(snippet) })
    }
  ...
  parse (string) {
    return this.bodyParser.parse(string)
  },

  provideSnippets () {
    return {
      parse: string => this.parse(string),
