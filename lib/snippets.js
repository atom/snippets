...
const ChoiceProvider = require('./autocomplete-choice')
...
module.exports = {
  activate () {
    ...
    this.snippetChoiceProvider = new ChoiceProvider
  ...
  deactivate () {
    ...
    this.snippetChoiceProvider.deactivate()
  ...
  expandSnippetsUnderCursors (editor) {
    ...
    editor.groupChangesSinceCheckpoint(checkpoint, {deleteCheckpoint: true})
  ...
  textChanged (editor, event) {
    ...
    this.ignoringTextChangesForEditor(editor, () => {
      let interval = editor.undoGroupingInterval === undefined ? 300 : editor.undoGroupingInterval // HACK: relies on private editor property, and on one that will likely change in future (to non time based undo)
  ...
  provideAutocomplete () {
    return this.snippetChoiceProvider
  }
}
