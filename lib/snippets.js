...
module.exports = {
  activate () {
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      'snippets:expand'(event) {
        ...
        const snippet = snippets.snippetToExpandUnderCursor(editor)
        if (snippet) {
          snippets.clearExpansions(editor)
          snippets.expandSnippetsUnderCursors(editor, snippet)
        } else {
  ...
  loadBundledSnippets (callback) {
    const bundledSnippetsPath = CSON.resolve(path.join(getPackageRoot(), 'snippets', 'snippets'))
  ...
  getParsedSnippet (attributes) {
    ...
    if (snippet == null) {
      ...
      snippet = new Snippet({id, name, prefix, bodyTree, description, descriptionMoreURL, rightLabelHTML, leftLabel, leftLabelHTML, bodyText: body, variableResolver: this.variableResolver, transformResolver: this.transformResolver})
  ...
  expandSnippetsUnderCursors (editor, snippet) {
    if (!snippet) {
      snippet = this.snippetToExpandUnderCursor(editor)
    }
    ...
    for (const cursor of cursors) {
      ...
      const startPoint = cursorPosition.translate([0, -snippet.prefix.length])
      const oldSelectionRange = cursor.selection.getBufferRange()
      cursor.selection.setBufferRange([startPoint, cursorPosition])
      this.insert(snippet, editor, cursor, oldSelectionRange)
    }
  ...
  goToNextTabStop (editor) {
    ...
    for (const expansion of this.getExpansions(editor)) {
      ...
      const {succeeded, isDestroyed} = expansion.goToNextTabStop()
      if (!destroy) { destroy = isDestroyed }
      if (succeeded) { nextTabStopVisited = true }
    }

    if (destroy) {
      this.destroyExpansions(editor)
    }

    if (nextTabStopVisited) {
      editor.buffer.createCheckpoint()
    }
  ...
  goToPreviousTabStop (editor) {
    ...
    for (const expansion of this.getExpansions(editor)) {
      if (!expansion) { continue; }
      const {succeeded} = expansion.goToPreviousTabStop(!previousTabStopVisited)
      if (succeeded) { previousTabStopVisited = true }
    }

    if (previousTabStopVisited) {
      editor.buffer.createCheckpoint()
    }
  ...
  destroyExpansions (editor) {
    this.findOrCreateMarkerLayer(editor).clear()
  ...
  textChanged (editor, event) {
    ...
    this.ignoringTextChangesForEditor(editor, () => {
      // HACK: relies on private editor property, and on one that will likely change in future (to non time based undo)
      const interval = editor.undoGroupingInterval == undefined ? 300 : editor.undoGroupingInterval
      editor.transact(interval, () =>
  ...
  insert (snippet, editor, cursor, oldSelectionRange) {
    ...
    if (typeof snippet === 'string') {
      ...
      snippet = new Snippet({name: '__anonymous', prefix: '', bodyTree, bodyText: snippet, variableResolver: this.variableResolver, transformResolver: this.transformResolver})
    }
    return new SnippetExpansion(snippet, editor, cursor, oldSelectionRange, this)
  },
  ...
  consumeResolver (payload) {
    if (payload === null || typeof payload !== 'object') return

    const variableResolvers = payload.variableResolvers
    const transformResolvers = payload.transformResolvers

    if (variableResolvers) {
      this.addResolvers(this.variableResolver, variableResolvers)
    }

    if (transformResolvers) {
      this.addResolvers(this.transformResolver, transformResolvers)
    }
  },

  addResolvers (resolverObject, nameResolverPairs) {
    let itr
    if (nameResolverPairs instanceof Map) {
      itr = nameResolverPairs.entries()
    } else if (typeof nameResolverPairs === 'object') {
      itr = Object.entries(nameResolverPairs)
    } else {
      return
    }

    for (const [varName, resolver] of itr) {
      if (typeof varName !== 'string' || typeof resolver !== 'function') continue
      resolverObject.add(varName, resolver)
    }
  }
}
