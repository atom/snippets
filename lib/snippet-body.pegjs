...
bodyContentChar
  = escaped
  / !tabStop char:. { return char; }

escaped = "\\" char:. { return char; }

tabStop
  = tabStopWithTransformation
...
tabStopWithPlaceholder
  = "${" index:[0-9]+ ":" content:placeholderContent "}" {
...
placeholderContentChar
  = escaped
  / placeholderVariableReference
  / !tabStop !variable char:[^}] { return char; }
...
transformationSubstitution
  = "/"
    find:(escapedForwardSlash / [^/])*
    "/"
...
// other tasks.
formatStringEscape = "\\" flag:[ULulErn$] { return { escape: flag }; }
