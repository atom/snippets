...
tabStop = tabStopWithTransformationAndPlaceholder / tabStopWithTransformation /  tabStopWithPlaceholder / tabStopWithoutPlaceholder / simpleTabStop
...
tabStopWithTransformation = '${' index:[0-9]+ substitution:substitution '}' {
  return {
...
tabStopWithTransformationAndPlaceholder = '${' index:[0-9]+ substitution:substitution ':' content:placeholderContent '}' {
  return {
    index: parseInt(index.join(""), 10),
    content: content,
    substitution: substitution
...
substitution = '/' find:([^/] / escapedForwardSlash)* '/' replace:formatString* '/' flags:[imy]* {
  let reFind = new RegExp(find.join(''), flags.join('') + 'g');
  return { find: reFind, replace: replace[0] };
}

formatString = content:(formatStringEscape / formatStringReference / escapedForwardSlash / [^/])+ { return content; }
